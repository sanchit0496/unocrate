...
class Message {
	...
	public function getMessages($otherUser){
		...
		while ($row=mysqli_fetch_array($get_messages_query)) {
			...
			$div_top = ($user_to == $userLoggedIn) ? "<div class='message' id='green'>" : 
	...
	public function getConvos() {
		...
		foreach($convos as $username) {
			...
			$return_string .= "<a href='messages.php?u=$username'> <div class='user_found_messages'>
								<img src='" . $user_found_obj->getProfilePic() . "' style='border-radius: 50%; margin-right: 10px;height:50px;width:50px;'>
								" . $user_found_obj->getFirstAndLastName() . " <br>
								<span class='timestamp_smaller' id='greymsgtime'> " . $latest_message_details[2] . "</span>
	...
	public function getConvosDropdown($data, $limit) {
		...
		foreach($convos as $username) {
			...
			$return_string .= "<a href='messages.php?u=$username'> 
								...
								<img src='" . $user_found_obj->getProfilePic() . "' style='border-radius: 50%; margin-right: 15px;'>
								" . $user_found_obj->getFirstAndLastName() . "
								<br><span class='timestamp_smaller' id='greymsgtime'> " . $latest_message_details[2] . "</span>
								<p id='grey' style='margin: 0;'>" . $latest_message_details[0] . $split . " </p>
								</div>
								</a>";
		}
